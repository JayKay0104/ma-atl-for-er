...
#        elif(nr1==nr2): 
#            return 1
#        # if they differ in only 10 then 0.5
#        elif ((abs(nr1-nr2))<10):
#            return 0.5
#        else: 
#            return 0
...
# not a complete list in general but includes all (at least almost) identified formats in the datasets
date_formats = ['%Y-%m-%d', '%Y-%m-%d %H:%M:%S', '(%d %b %Y)', '%d-%m-%Y', '%m-%d-%Y', '%m-%d-%y', '%d-%m-%y',
...
def years_diff(str1,str2):
    try:
        ...
        if (str1 == '' or str2 == '' or str1 == 'nan' or str2 == 'nan'):
            ...
            # we already aligned the string format
#            d1 = datetime.strptime(str1, get_date_format(str1))
#            d2 = datetime.strptime(str2, get_date_format(str2))
            # return the diff in years no matter which month and day, so 01.01.2002 and 31.12.2004
            # get a diff of 2 years. Month diff and day diff give more accurate numbers then
            # but if 2002 (so no clue about exact date) and 31.12.2004 also get 2. For days
            # and month diff it will be assumed it is the 01.01.2002 if only 2002 is provided
