...
def active_unsuper_bootstrapped_learning(candsets,target_name,feature,estimator_name,query_strategy,quota,warm_start,
                                         ids='id',agg_sim='sum_weighted_sim',reweight='score_based',disagreement='vote',n=5):
    """
    query_strategy: 
    ...
    X_target = candsets[target_name][feature+[agg_sim,ids]]
    y_target = candsets[target_name]['label']
    ...
    test_ds = Dataset(X=X_target_test.drop(columns=[agg_sim,ids]),y=y_target_test)
    ...
    for i in range(n):
        ...
        train_ds, fully_labeled_trn_ds, labeled_weight = initializeUnsupervisedPool(X_train,y_train,warm_start,
                                                                    ids, agg_sim, reweight)
...
def initializeUnsupervisedPool(X_target_train, y_target_train, warm_start, ids, agg_sim, reweight):
    print('Initialize Dataset Object')
    
    
    X_target_train['label'] = y_target_train
    ...
    elbow_th, index = elbow_threshold(np.array(X_target_train[agg_sim]))
    ...
    if reweight == 'score_based':
        ...
        ## new block above
        train_ds =  le.UnsupervisedPoolDataset(X_target_train.drop(columns=['noisy_label','label',agg_sim,ids]), pool_labels, pool_unsupervised_labels, ids, pool_unsupervised_weights, reweight)

    else: 
        train_ds =  le.UnsupervisedPoolDataset(X_target_train.drop(columns=['noisy_label','label',agg_sim,ids]), pool_labels, pool_unsupervised_labels, ids, reweight)
    

#    return pool,labeled_weight
    # here we have the fully labeled training set 
    fully_labeled_trn_ds = Dataset(X=X_target_train.drop(columns=['noisy_label','label',agg_sim,ids]),y=X_target_train['label'])
...
def al_unsuper_bootstrapped_different_settings_single(candsets,target_name,feature,estimators,query_strategies,quota,warm_start,
                                                      ids='id',agg_sim='sum_weighted_sim',reweight='score_based',
    ...
    for est in estimators:
        ...
        for qs in query_strategies:
            ...
            temp = active_unsuper_bootstrapped_learning(candsets,target_name,feature,est,qs,quota,
                                                        warm_start,ids,agg_sim,reweight,disagreement,n)
...
def active_learning_different_settings_all(candsets,feature,estimators,query_strategies,quota,
                                       warm_start,ids='id',agg_sim='sum_weighted_sim',reweight='score_based',disagreement='vote',n=5):
    
    al_results = {}
    for candset in candsets:
        print('Start with AL using different settings for {}'.format(candset))
        target_name = candset
        temp = al_unsuper_bootstrapped_different_settings_single(candsets,target_name,feature,estimators,query_strategies,quota,
                                                                 warm_start,ids,agg_sim,reweight,disagreement,n)
